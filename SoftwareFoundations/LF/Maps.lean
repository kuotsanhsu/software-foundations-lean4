...
_Maps_ (or _dictionaries_) are ubiquitous data structures both in ordinary programming and in the theory of programming languages; we're going to need them in many places in the coming chapters.
...
def TotalMap.{u} (α: Type u) := String → α
...
def TotalMap.empty {α} (a: α): TotalMap α | _ => a
...
def TotalMap.update {α} (m: TotalMap α) (x: String) (y: α) :=
  fun x' => if x = x' then y else m x'

/-!
This definition is a nice example of higher-order programming: `t_update` takes a _function_ `m` and yields a new function `fun x' => ...` that behaves like the desired map.

For example, we can build a map taking `String`s to `Bool`s, where `"foo"` and `"bar"` are mapped to `true` and every other key is mapped to `false`, like this:
-/

open TotalMap in
example: TotalMap Bool := update (update (empty false) "foo" true) "bar" true
...
Theorem t_update_neq : forall (A : Type) (m : total_map A) x1 x2 v,
  ...
  (x1 !-> v ; m) x2 = m x2.
Proof.
  intros. unfold t_update. apply eqb_neq in H. rewrite H. reflexivity.
Qed.
(** [] *)
...
Lemma t_update_shadow : forall (A : Type) (m : total_map A) x v1 v2,
  (x !-> v2 ; x !-> v1 ; m) = (x !-> v2 ; m).
Proof.
  intros. extensionality y. unfold t_update. destruct (x =? y)%string.
  - reflexivity.
  - reflexivity.
Qed.
(** [] *)
...
Theorem t_update_same : forall (A : Type) (m : total_map A) x,
  (x !-> m x ; m) = m.
Proof.
  intros. extensionality y. unfold t_update. destruct (eqb_spec x y).
  - f_equal. exact e.
  - reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  destruct (eqb_spec x1 y).
  - destruct (eqb_spec x2 y).
    + assert (h: x2 = x1). { rewrite <-e in e0. exact e0. }
      contradiction.
    + reflexivity.
  - reflexivity.
  (* destruct (eqb_spec x1 y), (eqb_spec x2 y).
  - assert (h: x2 = x1). { rewrite <-e in e0. exact e0. }
    contradiction.
  - reflexivity.
  - reflexivity.
