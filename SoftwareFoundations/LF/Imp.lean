import SoftwareFoundations.LF.Basics
import SoftwareFoundations.LF.Maps

/-!
# Imp: Simple Imperative Programs

In this chapter, we take a more serious look at how to use Lean as a tool to study other things. Our case study is a _simple imperative programming language_ called Imp, embodying a tiny core fragment of conventional mainstream languages such as C and Java.
...
We concentrate here on defining the _syntax_ and _semantics_ of Imp; later, in _Programming Language Foundations_ (_Software Foundations_, volume 2), we develop a theory of _program equivalence_ and introduce _Hoare Logic_, a popular logic for reasoning about imperative programs.

## Arithmetic and Boolean Expressions

We'll present Imp in three parts: first a core language of _arithmetic and boolean expressions_, then an extension of these with _variables_, and finally a language of _commands_ including assignment, conditionals, sequencing, and loops.

### Syntax
-/

namespace aexp_playground

/-!
These two definitions specify the _abstract syntax_ of arithmetic and boolean expressions.
-/
...
/-!
In this chapter, we'll mostly elide the translation from the concrete syntax that a programmer would actually write to these abstract syntax trees -- the process that, for example, would translate the string `"1 + 2 * 3"` to the AST `add (ofNat 1) (mul (ofNat 2) (ofNat 3))`.
...
- The BNF is more informal -- for example, it gives some suggestions about the surface syntax of expressions (like the fact that the addition operation is written with an infix `+`) while leaving other aspects of lexical analysis and parsing (like the relative precedence of `+`, `-`, and `*`, the use of parens to group subexpressions, etc.) unspecified. Some additional information -- and human intelligence -- would be required to turn this description into a formal definition, e.g., for implementing a compiler.
  The Lean version consistently omits all this information and concentrates on the abstract syntax only.
...
protected def AExp.eval: (a: AExp) → Nat
  | ofNat n => n
...
open AExp in
example: (add (ofNat 2) (ofNat 2)).eval = 4 := rfl
...
protected def BExp.eval: BExp → Bool
  | true      => .true
  | false     => .false
...
We haven't defined very much yet, but we can already get some mileage out of the definitions. Suppose we define a function that takes an arithmetic expression and slightly simplifies it, changing every occurrence of `0 + e` (i.e., `(add (ofNat 0) e`) into just `e`.
-/

def AExp.optimize_0plus: AExp → AExp
  | ofNat n => ofNat n
  | add e₁ (ofNat 0)  => e₁.optimize_0plus
  | add e₁ e₂ => add e₁.optimize_0plus e₂.optimize_0plus
  | sub e₁ e₂ => sub e₁.optimize_0plus e₂.optimize_0plus
  | mul e₁ e₂ => mul e₁.optimize_0plus e₂.optimize_0plus
...
example: (
    add (ofNat 2) (
      add (add (ofNat 1) (ofNat 0)) (ofNat 0) 
    )
  ).optimize_0plus = add (ofNat 2) (ofNat 1) := rfl

/-!
But if we want to be certain the optimization is correct -- that evaluating an optimized expression _always_ gives the same result as the original -- we should prove it!
-/

theorem AExp.optimize_0plus_sound (e: AExp): e.optimize_0plus.eval = e.eval
:= by
  induction e with
  | ofNat n => rfl
  | add e₁ e₂ ih₁ ih₂ =>
    ...
    match e₂ with
    | ofNat 0 => rw [ih₁]; rfl
    | ofNat (.succ n) => simp; unfold AExp.eval; rw [ih₁, ih₂]
    | add ..
    | sub ..
    | mul .. => unfold AExp.eval; rw [ih₁, ih₂]
  | sub e₁ e₂ ih₁ ih₂
  | mul e₁ e₂ ih₁ ih₂ => unfold optimize_0plus; unfold AExp.eval; rw [ih₁, ih₂]
...
So far, we've been doing all our proofs using just a small handful of Lean's tactics and completely ignoring its powerful facilities for constructing parts of proofs automatically. This section introduces some of these facilities, and we will see more over the next several chapters. Getting used to them will take some energy -- Lean's automation is a power tool -- but it will allow us to scale up our efforts to more complex definitions and more interesting properties without becoming overwhelmed by boring, repetitive, low-level details.

### Tactic Combinators

[_Tactical combinators_](https://leanprover.github.io/theorem_proving_in_lean4/tactics.html#tactic-combinators) is Lean's term for tactics that take other tactics as arguments -- "higher-order tactics," if you will.

#### The `try` Tactic Combinator

If `T` is a tactic, then `try T` is a tactic that is just like `T` except that, if `T` fails, `try T` _successfully_ does nothing at all (rather than failing).
-/

example (e: AExp): e.eval = e.eval := by
  try rfl -- This just does `rfl`.

example (p: Prop): p → p := by
  intros h
  try rfl -- Plain `rfl` would have failed.
  apply h -- We can still finish the proof in some other way.

/-!
There is not much reason to use `try` in completely manual proofs like these, but it is very useful for doing automated proofs in conjunction with the `;` tactic combinator, which we show next.

### The `<;>` Tactic Combinator (Simple Form)

In its most common form, the `<;>` tactic combinator takes two tactics as arguments. The compound tactic `T <;> T'` first performs `T` and then performs `T'` on _each subgoal_ generated by `T`.
...
/-!
We can simplify this proof using the `<;>` tactic combinator:
-/

example (n: Nat): (0 ≤? n) = true := by
  -- destruct the current goal
  cases n
  -- then do `rfl` on each resulting subgoal
  <;> rfl

/-!
Using `try` and `<;>` together, we can get rid of the repetition in the proof of `optimize_0plus_sound` that was bothering us a little while ago.
-/

example (e: AExp): e.optimize_0plus.eval = e.eval := by
  sorry
/-
Proof.
  intros a.
...
(** Lean experts often use this "[...; try... ]" idiom after a tactic
...
/-!
However, this proof can still be improved: the first case (for `a = ofNat n`) is very trivial -- even more trivial than the cases that we said simply followed from the `ih` -- yet we have chosen to write it out in full. It would be better and clearer to drop it and just say, at the top, "Most cases are either immediate or direct from the `ih`. The only interesting case is the one for `add`..."  We can make the same improvement in our formal proof too.  Here's how it looks:
-/


example (e: AExp): e.optimize_0plus.eval = e.eval := sorry
  -- induction a
  --   -- Most cases follow directly by the IH
  --   try (by unfold optimize_0plus; unfold eval; rw [ih₁, ih₂]) <;>
...
/-
#### The `<;>` Tactic Combinator (General Form)

Doesn't exist for Lean.

#### The `repeat` Tactic Combinator

The `repeat` tactic combinator takes another tactic and keeps applying this tactic until it fails or until it succeeds but doesn't make any progress.
...
Proof.
  ...
  (* Uncomment the next line to see the infinite loop occur.  You will
     then need to interrupt Lean to make it listen to you again.  (In
...
(** Wait -- did we just write an infinite loop in Coq?!?!
    ...
    While evaluation in Lean's term language, Lean code, is guaranteed to
...
(** **** Exercise: 3 stars, standard (optimize_0plus_b_sound)
    ...
    it is sound.  Use the tactic combinators we've just seen to make the proof
...
(** Lean also provides facilities for "programming" in tactic
    scripts.
    ...
    Just for future reference: Lean provides two other ways of defining
    ...
    build tactics that directly manipulate Lean's internal structures.
...
(** (Note the [From Lean Require Import Lia.] at the top of
...
We have presented `AExp.eval` and `BExp.eval` as functions defined by `def`s.  Another way to think about evaluation -- one that we will see is often more flexible -- is as a _relation_ between expressions and their values. This leads naturally to `inductive` definitions like the following...
-/

protected inductive AExp.Eval: AExp → Nat → Prop
  | ofNat n: AExp.Eval (ofNat n) n
  | add {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → AExp.Eval (add e₁ e₂) (n₁ + n₂)
  | sub {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → AExp.Eval (sub e₁ e₂) (n₁ - n₂)
  | mul {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → AExp.Eval (mul e₁ e₂) (n₁ * n₂)

namespace HypothesisNames

/-!
A small notational aside. We could also write the definition of `AExp.Eval` as follow, with explicit names for the hypotheses in each case:
-/

open AExp in
inductive Eval: AExp → Nat → Prop
  | ofNat (n: Nat): Eval (ofNat n) n
  | add {e₁ e₂ n₁ n₂}
      (h₁: Eval e₁ n₁)
      (h₂: Eval e₂ n₂)
    : Eval (add e₁ e₂) (n₁ + n₂)
...
/-!
This style gives us more control over the names that Lean chooses during proofs involving `AExp.Eval`, at the cost of making the definition a little more verbose.
-/

end HypothesisNames

/-!
It will be convenient to have an infix notation for `AExp.Eval`. We'll write `e ==> n` to mean that arithmetic expression `e` evaluates to value `n`. Note that Lean doesn't support forward "elaboration" of notations nor macros like the pairing of `Reverse Notaion "..."` and `inductive | ... where "..." := ...` in Coq.
-/

class EvalRel (α β) where
  evalRel: α → β → Prop

infixl:40 " ==> " => EvalRel.evalRel

instance: EvalRel AExp Nat where
  evalRel := AExp.Eval

/-!
### Inference Rule Notation

In informal discussions, it is convenient to write the rules for `AExp.Eval` and similar relations in the more readable graphical form of _inference rules_, where the premises above the line justify the conclusion below the line.

For example, the constructor `add`...
```lean
| add (e₁ e₂: AExp) (n₁ n₂: Nat)
  : Eval e₁ n₁
  → Eval e₂ n₂
  → Eval (add e₁ e₂) (n₁ + n₂)
```
...
All the variables mentioned in the rule (`e₁`, `n₁`, etc.) are implicitly bound by universal quantifiers at the beginning. (Such variables are often called _metavariables_ to distinguish them from the variables of the language we are defining. At the moment, our arithmetic expressions don't include variables, but we'll soon be adding them.)

The whole collection of rules is understood as being wrapped in an `inductive` declaration. In informal prose, this is sometimes indicated by saying something like "Let `AExp.Eval` be the smallest relation closed under the following rules...".
...
Here, again, is the Lean definition of the `BExp.eval` function:
-/
#print BExp.eval
...
-/
-- FILL IN HERE
protected inductive BExp.Eval: BExp → Bool → Prop
  | true: BExp.Eval true .true
  | false: BExp.Eval false .false
  | eq {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → BExp.Eval (eq e₁ e₂) (n₁ =? n₂)
  | neq {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → BExp.Eval (neq e₁ e₂) !(n₁ =? n₂)
  | le {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → BExp.Eval (le e₁ e₂) (n₁ ≤? n₂)
  | gt {e₁ e₂ n₁ n₂}
    : AExp.Eval e₁ n₁
    → AExp.Eval e₂ n₂
    → BExp.Eval (gt e₁ e₂) !(n₁ ≤? n₂)
  | not {e b}
    : BExp.Eval e b
    → BExp.Eval (not e) !b
  | and {e₁ e₂ b₁ b₂}
    : BExp.Eval e₁ b₁
    → BExp.Eval e₂ b₂
    → BExp.Eval (and e₁ e₂) (b₁ && b₂)

/-!
Do not modify the following line:
-/
def manual_grade_for_beval_rules: Option (Nat × String) := none
-- ∎
...
theorem AExp.Eval_iff_eval (e: AExp) (n: Nat): e ==> n ↔ e.eval = n := by
  constructor
  . intro h
    induction h with
    | ofNat => rfl
    | add _ _ ih₁ ih₂ => rw [←ih₁, ←ih₂]; rfl
    | sub _ _ ih₁ ih₂ => rw [←ih₁, ←ih₂]; rfl
    | mul _ _ ih₁ ih₂ => rw [←ih₁, ←ih₂]; rfl
  . intro h; subst h
    induction e with
    | ofNat n => constructor
    | add _ _ ih₁ ih₂ => constructor; exact ih₁; exact ih₂
    | sub _ _ ih₁ ih₂ => constructor; exact ih₁; exact ih₂
    | mul _ _ ih₁ ih₂ => constructor; exact ih₁; exact ih₂

/-!
Again, we can make the proof quite a bit shorter using some tactics.
-/

example (e: AExp) (n: Nat): e ==> n ↔ e.eval = n := by
  constructor <;> intro h
  . induction h <;> first | rfl | (next ih₁ ih₂ => rw [←ih₁, ←ih₂]; rfl)
  . subst h; induction e <;> constructor <;> assumption

/-!
#### Exercise: 3 stars, standard (bevalR)

Write a relation `BExp.Eval` in the same style as `AExp.Eval`, and prove that it is equivalent to `BExp.eval`.
-/

-- inductive BExp.Eval: BExp → Bool → Prop := sorry
-- FILL IN HERE

instance: EvalRel BExp Bool where
  evalRel := BExp.Eval

theorem BExp.Eval_iff_eval (e: BExp) (b: Bool): e ==> b ↔ e.eval = b := by
  constructor <;> intro h
  . induction h with
    | true | false => rfl
    | eq ih₁ ih₂ | neq ih₁ ih₂ | le ih₁ ih₂ | gt ih₁ ih₂ =>
      unfold BExp.eval
      rw [(AExp.Eval_iff_eval _ _).mp ih₁, (AExp.Eval_iff_eval _ _).mp ih₂]
    | not _ ih => rw [←ih]; rfl
    | and _ _ ih₁ ih₂ => rw [←ih₁, ←ih₂]; rfl
  . subst h; induction e <;> constructor <;>
    first | assumption | exact (AExp.Eval_iff_eval _ _).mpr rfl
-- ∎
...
/-!
Extending the definition of `AExp.eval` to handle this new operation would not be straightforward (what should we return as the result of `div (ofNat 5) (ofNat 0)`?). But extending `AExp.Eval` is very easy.
-/

inductive AExp.Eval: AExp → Nat → Prop
  | ofNat n: Eval (ofNat n) n
  | add {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (add e₁ e₂) (n₁ + n₂)
  | sub {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (sub e₁ e₂) (n₁ - n₂)
  | mul {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (mul e₁ e₂) (n₁ * n₂)
  | div {e₁ e₂ n₁ n₂ n₃} -- NEW
    : Eval e₁ n₁ → Eval e₂ n₂ → n₃ ≠ 0 → n₁ = n₂ * n₃ → Eval (div e₁ e₂) n₃

/-!
Notice that the evaluation relation has become _partial_: There are some inputs for which it does not specify an output.
-/
...
(Note that this is not the same as making a _probabilistic_ choice among all possible numbers -- we're not specifying any particular probability distribution for the results, just saying what results are _possible_.)
-/
...
/-!
Again, extending `AExp.eval` would be tricky, since now evaluation is _not_ a deterministic function from expressions to numbers; but extending `AExp.eval` is no problem...
-/

inductive AExp.Eval: AExp → Nat → Prop
  | any n: Eval any n -- NEW
  | ofNat n: Eval (ofNat n) n
  | add {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (add e₁ e₂) (n₁ + n₂)
  | sub {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (sub e₁ e₂) (n₁ - n₂)
  | mul {e₁ e₂ n₁ n₂}: Eval e₁ n₁ → Eval e₂ n₂ → Eval (mul e₁ e₂) (n₁ * n₂)
...
One point in favor of relational definitions is that they can be smore elegant and easier to understand.

Another is that Lean automatically generates nice inversion and induction principles from `inductive` definitions.

On the other hand, functional definitions can often be more convenient:
- Functions are automatically deterministic and total; for a relational definition, we have to _prove_ these properties explicitly if we need them.
- With functions we can also take advantage of Lean's computation mechanism to simplify expressions during proofs.

Furthermore, functions can be directly "extracted" from Lean code to executable code in C.

Ultimately, the choice often comes down to either the specifics of a particular situation or simply a question of taste. Indeed, in large Lean developments it is common to see a definition given in _both_ functional and relational styles, plus a lemma stating that the two coincide, allowing further proofs to switch from one point of view to the other at will.
-/

def Lean.TSyntax.toTerm (s: Ident): Term := quote (toString s.getId)
...
A _machine state_ (or just _state_) represents the current values of all variables at some point in the execution of a program.

For simplicity, we assume that the state is defined for _all_ variables, even though any given program is only able to mention a finite number of them.  Because each variable stores a natural number, we can represent the state as a total map from strings (variable names) to `Nat`, and will use `0` as default value in the store.
-/
...
inductive AExp
  | ofNat (n: Nat)
  | id (s: String) -- NEW, "id" is short for "identifier"
  | add (e₁ e₂: AExp)
  | sub (e₁ e₂: AExp)
  | mul (e₁ e₂: AExp)
...
- The `Coe` declaration stipulates that a function (or constructor) can be implicitly used by the type system to coerce a value of the input type to a value of the output type. For instance, the coercion declaration for `AExp.id` allows us to use plain strings when an `AExp` is expected; the string will implicitly be wrapped with `AExp.id`.
- `Declare Custom Entry com` tells Lean to create a new "custom grammar" for parsing Imp expressions and programs. The first notation declaration after this tells Lean that anything between `<{` and `}>` should be parsed using sthe Imp grammar. Again, it is not necessary to understand the details, but it is important to recognize that we are defining _new_ interpretations for some familiar operators like `+`, `-`, `*`, `=`, `≤`, etc., when they occur between `<{` and `}>`.
-/
...
-- https://github.com/arthurpaulino/lean4-metaprogramming-book/blob/master/lean/main/dsls.lean
...
syntax:50 imp " = " imp : imp
syntax:50 imp " ≠ " imp : imp
syntax:50 imp " ≤ " imp : imp
syntax:50 imp " > " imp : imp
syntax:max "¬" imp:40 : imp
syntax:35 imp:36 " ∧ " imp:35 : imp

macro_rules
  | `(<{($e)}>) => `(<{$e}>)
  | `(<{$n:num}>) => `(AExp.ofNat $n)
  -- Matchers for constants must come before the identifier matcher.
  | `(<{true}>) => `(BExp.true)
  | `(<{false}>) => `(BExp.false)
  | `(<{$s:ident}>) => `(AExp.id $(s.toTerm))
  | `(<{$e₁ + $e₂}>) => `(AExp.add <{$e₁}> <{$e₂}>)
  | `(<{$e₁ - $e₂}>) => `(AExp.sub <{$e₁}> <{$e₂}>)
  | `(<{$e₁ * $e₂}>) => `(AExp.mul <{$e₁}> <{$e₂}>)
  | `(<{$e₁ = $e₂}>) => `(BExp.eq  <{$e₁}> <{$e₂}>)
...
/-!
We can now write `3 + (X * 2)` instead  of `add 3 (mul X 2)`, and `true && ~(X ≤ 4)` instead of `and true (not (le X 4))`
-/

#reduce (<{ 3 + (X * 2) }> : AExp)
#reduce (<{ true ∧ ¬(X ≤ 4) }> : BExp)
...
protected def AExp.eval (/- NEW -/ st: State): AExp → Nat
  | (n: Nat) => n
  | (s: String) => st s -- NEW
  | e₁ + e₂ => e₁.eval st + e₂.eval st
  | e₁ - e₂ => e₁.eval st - e₂.eval st
  | e₁ * e₂ => e₁.eval st * e₂.eval st

protected def BExp.eval (/- NEW -/ st: State): BExp → Bool
  | true     => .true
  | false    => .false
  | eq  e₁ e₂ => e₁.eval st =? e₂.eval st
  | neq e₁ e₂ => !(e₁.eval st =? e₂.eval st)
  | le  e₁ e₂ => e₁.eval st ≤? e₂.eval st
  | gt  e₁ e₂ => !(e₁.eval st ≤? e₂.eval st)
  | not e     => !e.eval st
  | and e₁ e₂ => e₁.eval st && e₂.eval st

/-!
Let's define the empty state as `TotalMap.empty 0`. Also, we can add a notation for a "singleton state" with just one variable bound to a value.
-/

macro x:term " !-> " y:term : term =>
  `(TotalMap.update (TotalMap.empty 0) $x $y)
macro x:term " !-> " y:term " ; " m:term : term => `(TotalMap.update $m $x $y)

example: <{ 3 + (X * 2) }>.eval ("X" !-> 5) = 13 := rfl
example: <{ Z + (X * Y) }>.eval ("X" !-> 5 ; "Y" !-> 4) = 20 := rfl
example: <{ true ∧ ¬(X ≤ 4) }>.eval ("X" !-> 5) = true := rfl

/-
## Commands

Now we are ready to define the syntax and behavior of Imp _commands_ (or _statements_).
...
Here is the formal definition of the abstract syntax of scommands:
-/

inductive Com
  | skip
  | assign (s: String) (e: AExp)
  | seq (c₁ c₂: Com)
  | if (e: BExp) (c₁ c₂: Com)
  | while (e: BExp) (c: Com)

/-!
As we did for expressions, we can use a few `syntax` declarations to make reading and writing Imp programs more convenient.
-/
...
macro_rules
  | `(<{skip}>) => `(Com.skip)
  | `(<{$s:ident := $e}>) =>
    `(Com.assign $(s.toTerm) <{$e}>)
  | `(<{$c₁ ; $c₂}>) => `(Com.seq <{$c₁}> <{$c₂}>)
  | `(<{if $e then $c₁ else $c₂ end}>) => `(Com.if <{$e}> <{$c₁}> <{$c₂}>)
  | `(<{while $e do $c end}>) => `(Com.while <{$e}> <{$c}>)

/-!
For example, here is the factorial function again, written as a formal Lean definition. When this command terminates, the variable `Y` will contain the factorial of the initial value of `X`.
-/
...
(** In a more conventional functional programming language like OCaml or
    ...
    Lean doesn't accept such a definition ("Error: Cannot guess
    ...
    terminate. Since Lean aims to be not just a functional programming
    language but also a consistent logic, any potentially
    non-terminating function needs to be rejected.

    Here is an example showing what would go wrong if Lean allowed
    ...
    a disaster for Lean's logical consistency.

    Thus, because it doesn't terminate on all inputs, [ceval_fun]
    cannot be written in Lean -- at least not without additional tricks
...
(** The cost of defining evaluation as a relation instead of a
    ...
    Lean's computation mechanism do it for us. *)
...
Proof.
  ...
  (** Inverting [Heval] essentially forces Lean to expand one step of
