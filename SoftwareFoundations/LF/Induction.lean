...
theorem Nat.double_plus: (n: Nat) → n.double = n + n
  | 0 => rfl
  | succ n =>
    calc n.double.succ.succ
       = (n + n).succ.succ := congrArg (·.succ.succ) n.double_plus
     _ = (n + n.succ).succ := rfl
     _ = n.succ + n.succ := (succ_add n n.succ).symm
-- (** [] *)
...
[induction] is [destruction] with additional access to the inductive hypothesis.
*)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    Proof: Induce on [n] where the base case is exactly [add_0_r]. The inductive
    step follows from [plus_n_Sm] applied to the inductive hypothesis.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    Proof: Induction by applying [reflexivity] at every step.
*)
...
Proof.
  ...
  rewrite add_assoc, (add_comm n m), <-add_assoc.
  reflexivity.
Qed.
...
Proof.
  ...
  - exact (mult_n_O _).
  - simpl.
    assert (∀ m n: nat, n * S m = n + n * m) as H. {
      intros x y.
      induction y as [| y IHy ].
      + reflexivity.
      + simpl. rewrite IHy, add_shuffle3. reflexivity.
    }
    simpl. rewrite IHm, H. reflexivity.
Qed.  
(** [] *)
...
Proof. intros. simpl. exact (add_0_r n). Qed.
Check eq_sym.
...
/- (FIXME)
Before you start working on the next exercise, replace the stub definitions of `succ` and `toNat`, below, with your solution from `Basics`.  That will make it possible for this file to be graded on its own.
-/

/-!
In `Basics`, we did some unit testing of `toNat`, but we didn't prove its correctness. Now we'll do so.
...
If you want to change your previous definitions of `succ` or `toNat` to make the property easier to prove, feel free to do so!
-/

theorem Bin.toNat_pres_succ
  : (b: Bin) → b.succ.toNat = b.toNat.succ
  | Z | B₀ _ => rfl
  | B₁ b => congrArg (· * 2) b.toNat_pres_succ
...
Hint: This proof should go through smoothly using the previous exercise about `succ` as a lemma. If not, revisit your definitions of the functions involved and consider whether they are more complicated than necessary: the shape of a proof by induction will match the recursive structure of the program being verified, so make the recursions as simple as possible.
-/

theorem Nat.nat_bin_nat: (n: Nat) → n.toBin.toNat = n
  | 0 => rfl
  | succ n =>
    calc n.toBin.succ.toNat
       = n.toBin.toNat.succ := n.toBin.toNat_pres_succ
     _ = n.succ := congrArg succ n.nat_bin_nat
...
example (b: Bin): b.toNat.toBin = b := sorry
...
open Bin in
example: Z.double = Z := rfl

/-! Prove this lemma, which corresponds to `double_succ`. -/

theorem Bin.double_succ: (b: Bin) → b.succ.double = b.double.succ.succ
  | Z | B₀ _ => rfl
  | B₁ b => congrArg B₀ b.double_succ
...
/-!
The theorem fails because there are some `Bin` such that we won't necessarily get back to the _original_ `Bin`, but instead to an "equivalent" `Bin`. (We deliberately leave that notion undefined here for you to think about.)

Explain in a comment, below, why this failure occurs. Your explanation will not be graded, but it's important that you get it clear in your mind before going on to the next part. If you're stuck on this, think about alternative implementations of `Bin.double` that might have failed to satisfy `double_bin_zero` yet otherwise seem correct.

(* FILL IN HERE *)


To solve that problem, we can introduce a _normalization_ function that selects the simplest `Bin` out of all the equivalent `Bin`. Then we can prove that the conversion from `Bin` to `Nat` and back again produces that normalized, simplest `Bin`.

### Exercise: 4 stars, advanced (bin_nat_bin)

Define `normalize`. You will need to keep its definition as simple as possible for later proofs to go smoothly. Do not use `Bin.toNat` or `Nat.toBin`, but do use `Bin.double`.

Hint: Structure the recursion such that it _always_ reaches the end of the `Bin` and process each bit only once. Do not try to "look ahead" at future bits.
-/

def Bin.normalize: Bin → Bin
  | Z => Z
  | B₀ b => b.normalize.double
  | B₁ b => b.normalize.double.succ
...
Hint 2: Lemma `double_succ` that you proved above will be helpful, too.
-/
theorem Bin.bin_nat_bin (b: Bin): b.toNat.toBin = b.normalize :=
  match b with
  | Z => rfl
  | B₀ b => aux b
  | B₁ b => congrArg succ (aux b)
where aux (b: Bin): (0 + b.toNat + b.toNat).toBin = b.normalize.double
:=
  have f (n: Nat): n.double.toBin = n.toBin.double :=
    match n with
    | 0 => rfl
    | .succ n =>
              calc n.double.toBin.succ.succ = n.toBin.succ.double := sorry
  this
  sorry
-- (** [] *)

theorem f: (n: Nat) → n.double.toBin = n.toBin.double
  | 0 => rfl
  | .succ n =>
    calc n.double.toBin.succ.succ = n.toBin.succ.double := congrArg (·.succ.succ) (f n)
