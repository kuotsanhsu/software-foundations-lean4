...
Other common features of functional languages include _algebraic data types_ and _pattern matching_, which make it easy to construct and manipulate rich data structures, and _polymorphic type systems_ supporting abstraction and code reuse. Lean offers all of these features.

The first half of this chapter introduces the most essential elements of Lean's native functional programming language features. The second half introduces some basic _tactics_ that can be used to prove properties of programs.

## Data and Functions

### Enumerated Types

One notable aspect of Lean is that its set of built-in features is _extremely_ small. For example, instead of providing the usual palette of atomic data types (booleans, integers, strings, etc.), Lean offers a powerful mechanism for defining new data types from scratch, with all these familiar types as instances.

Naturally, the Lean distribution comes with an extensive standard library providing definitions of booleans, numbers, and many common data structures like lists and hash tables. But there is nothing magic or primitive about these library definitions. To illustrate this, in this course we will explicitly recapitulate (almost) all the definitions we need, rather than getting them from the standard library.

### Days of the Week

To see how this definition mechanism works, let's start with a very simple example. The following declaration tells Lean that we are defining a set of data values - a _type_.
-/
...
namespace Day
/-!
The new type is called `Day`, and its members are `Monday`, `Tuesday`, etc. Having defined `Day`, we can write functions that operate on days.
-/
...
Having defined a function, we should next check that it works on some examples. There are actually three different ways to do the examples in Lean. First, we can use the command `#eval` to evaluate a compound expression involving `next_weekday`.
-/
...
:= match n with 0 => 1 | S m => mult n (factorial m) end.
...
(** It is worth stepping through these proofs to observe how the
    ...
    before [reflexivity] to see the simplifications that Lean performs
...
(** The [Admitted] command tells Lean that we want to skip trying
...
(** Note that the [destruct] here has no [as] clause because
    ...
    the [as] clause from _any_ [destruct] and Lean will fill in
...
(** Before closing the chapter, let's mention one final
    ...
    This pattern is so common that Lean provides a shorthand for it: we
...
(** For each notation symbol in Lean, we can specify its _precedence
    ...
    specified by writing [at level n]; this helps Lean parse compound
    expressions.  The associativity setting helps to disambiguate
    expressions containing multiple occurrences of the same
    symbol. For example, the parameters specified above for [+] and
    [*] say that the expression [1+2*3*4] is shorthand for
    [(1+((2*3)*4))]. Lean uses precedence levels from 0 to 100, and
    ...
    Lean tries to guess what scope is meant from context, so when it
    ...
    percent-notation by writing [(x*y)%nat], and sometimes in what Lean
    ...
    Pro tip: Lean's notation mechanism is not especially powerful.
...
(** When Lean checks this definition, it notes that [plus'] is
    "decreasing on 1st argument."  What this means is that we are
    performing a _structural recursion_ over the argument [n] - i.e.,
    that we make recursive calls only on strictly smaller values of
    [n].  This implies that all calls to [plus'] will eventually
    terminate.  Lean demands that some argument of _every_ [Fixpoint]
...
Proof.
  intros [] [] H.
  - reflexivity.
  - rewrite (eq_refl: false = true && false), H. reflexivity.
  - rewrite (eq_refl: false = false && true), H. reflexivity.
  - reflexivity.
Qed.
